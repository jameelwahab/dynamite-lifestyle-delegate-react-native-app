  ...
  if (res.code == 200) {
    ...
    dispatch(
      setUserAndToken({
        ...
        isSyncWithGoogleAllowed:
          res?.site_setting?.is_calendar_enabled_for_delegate,
        googleSyncedData: res?.consultant?.is_google_signin
          ? res?.consultant?.google_account_info
          : null,
    ...
    dispatch(
      setTimeZone({user: res?.consultant?.time_zone, admin: res?.time_zone}),
    );
    dispatch(
      setSocket(
        io(socketUrl, {
