...
const ReviewFeeds = ({navigation, route}) => {
  ...
  const getFeeds = async () => {
    const res = await GET_REVIEW_FEEDS({
      token,
      navigation,
      limit: 20,
      page: paging?.page,
    ...
    if (res.code == 200) {
      ...
      let length =
        paging?.page == 0
          ? res?.feeds.length
          : result.length + res?.feeds.length;
...
const renderPosts = ({feed, index, handleClick, onDetail}) => {
  ...
  return (
    <View
      ...
      />
      <View style={{height: 10}} />
      <TouchableOpacity onPress={onDetail} style={{alignSelf: 'flex-end'}}>
        <MyText underlined fontSize={12} color={colors.primary}>
          View Detail
        </MyText>
      </TouchableOpacity>
    </View>
