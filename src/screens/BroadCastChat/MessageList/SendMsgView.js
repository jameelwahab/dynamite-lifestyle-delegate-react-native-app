...
const SendMsgView = ({
    ...
    if (Platform.OS === 'android') {
      try {
        ...
        if (Platform.Version >= 33) {
          grants = await requestMultiple([PERMISSIONS.ANDROID.RECORD_AUDIO]);
        } else {
  ...
  const startRecording = async () => {
    try {
      const generateAudioName = () => {
        if (Platform.OS == 'android')
          return (
            ReactNativeBlobUtil.fs.dirs.CacheDir + '/' + moment().valueOf()
          );
      ...
      audioRecorderPlayer.addRecordBackListener(e => {
        ...
        if (
          ...
          moment.utc(e.currentPosition).format('mm:ss')
        ) {
          setRecorderTime({
  ...
  const sendMessage = async () => {
    ...
    let audioPath = '';
    if (!!msg.image?.uri) {
      imagePath = await uplaodFileOnS3(msg.image, 'image').then(
        res => res.image_path,
      );
      if (!!imagePath == false) {
      ...
      audioPath = await uplaodFileOnS3(recordedAudio.audio, 'audio').then(
        res => res.image_path,
      );
      if (!!audioPath == false) {
        setSendMsgLoader(false);
    ...
    if (!!edit?._id) {
      ...
      if (broadcastType.type == 2) {
        ...
        postData['schedule_time'] = moment(broadcastType.scheduleDate).format(
          'HH:mm',
        );
        postData['schedule_date'] = moment(broadcastType.scheduleDate).format(
          'YYYY-MM-DD',
        );
        postData['schedule_date_time'] = moment(
          broadcastType.scheduleDate,
      ...
      if (broadcastType.type == 2) {
        postData['schedule_time'] = moment(
          moment(broadcastType.scheduleDate).format(dateTimeFormat.date) +
            ' ' +
            broadcastType.scheduleTime,
          'DD-MM-YYYY HH:mm',
        ).toDate();
        postData['schedule_date'] = moment(
          moment(broadcastType.scheduleDate).format(dateTimeFormat.date),
          'DD-MM-YYYY',
        ).toDate();
        postData['schedule_date_time'] = moment(
        ...
        <SafeAreaView>
          <View style={__style.broadcastModalContent}>
              ...
              <View style={__style.dateTimeRow}>
                <View style={__style.flex1}>
                  <MyTouchableInput
                    ...
                    value={moment(broadcastType?.scheduleDate).format(
                      dateTimeFormat.date,
                    )}
                  />
                </View>
                <View style={__style.flex1MarginLeft}>
                  <MyTouchableInput
                    ...
                    value={moment(broadcastType?.scheduleTime, 'HH:mm').format(
                      dateTimeFormat.time,
                    )}
                  />
                </View>
            ...
            <View style={__style.personalNoteContainer}>
              <MyCheckBox
                ...
                onPress={() =>
                  setBroadcastType({
                    ...broadcastType,
                    addAsNote: !broadcastType?.addAsNote,
                  })
                }
              />
            </View>
            ...
            <CalendarModal
              ref={ref_calendar}
              onDateSelected={date =>
                setBroadcastType({...broadcastType, scheduleDate: date})
              }
            />
            ...
            <TimePicker
              ref={ref_timePicker}
              onAgree={time =>
                setBroadcastType({...broadcastType, scheduleTime: time})
              }
            />
          </View>
        </SafeAreaView>
  ...
  let displayImage = !!msg.image?.uri
    ? msg?.image?.uri
    : !!msg?.image
    ? S3_URL + msg?.image
    : '';
      ...
      {isRecording ? (
        ...
        <>
          ...
          <View style={__style.inputContainer}>
            <View style={__style.sendMsgInputView}>
              ...
              <Collapsible collapsed={isEditorVisible}>
                <View style={__style.editorView}>
                  ...
                  <TouchableOpacity
                    onPress={() =>
                      setLinkModal({title: '', link: '', isVisible: true})
                    }
                    style={__style.editorBotton}>
            ...
            <TouchableOpacity
              onPress={
                !!msg.text.trim() || !!msg.image ? sendMsgButton : recordBtn
              }
              disabled={fileLoader}
              style={__style.sendButtonView}>
              {fileLoader ? (
