...
import { IS_CHAT_EXIST, LIST_OF_MEMBERS } from '../../../DAL'
  ...
  const topView = () => {
    return (
      <View style={{ flexDirection: "row", flex: 1, marginHorizontal: 15, alignItems: "center", }}>
        <View>
          <MyText fontSize={18} type='bold' color={colors.primary} >{"All Members"}</MyText>
        </View>
  ...
  const headerView = () => {
    return (
      <View style={{ paddingHorizontal: 5, backgroundColor: colors.darkSecondary }}>
        {console.log(filterData?.sale_pages.find((x) => x._id == Filter?.event_page[0], "hi"))}
        <View style={__styles.allChipView}>
          <MyText type='mediym' >{"Filtered By : "}</MyText>
    ...
    return (
      <View style={__styles.memberRootView}>
        ...
        <View>
          ...
          <StatView title={"Membership Expire"} value={convertTimezone(item?.createdAt, timezone).format(dateTimeFormat.date)} />
        </View>
