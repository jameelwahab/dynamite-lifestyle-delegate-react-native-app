...
const MemberTickets = ({navigation, route}) => {
  ...
  const getSubscriptionListFromServer = async (firstTime = false) => {
    try {
      let res = await MEMBER_TICKETS_LIST({
        token,
        navigation,
        transId: route?.params?.transactionId,
      });
      if (res.code == 200) {
        let listLength = firstTime
          ? 0 + res?.transaction?.tickets.length
          : list.length + res?.transaction?.tickets.length;
        ...
        setList(
          firstTime
            ? res?.transaction?.tickets
            : [...list, ...res?.transaction?.tickets],
        );
      ...
      <View style={styles.listItemContainer}>
        <View style={styles.listItemHeader}>
          <MyText color={colors.primary}>
            {STRINGS.MEMBER_TICKETS.indexLabel(index)}
          </MyText>
        </View>
        <StatView
          title={STRINGS.MEMBER_TICKETS.firstName}
          value={item?.first_name}
        />
        <StatView
          title={STRINGS.MEMBER_TICKETS.lastName}
          value={item?.last_name}
        />
        <StatView title={STRINGS.MEMBER_TICKETS.email} value={item?.email} />
        <StatView
          title={STRINGS.MEMBER_TICKETS.phone}
          value={item?.contact_number}
        />
        <StatView
          title={STRINGS.MEMBER_TICKETS.venue}
          value={item?.venue?.title}
        />
      </View>
    );
  };
  ...
  return (
    <RootView title={STRINGS.MEMBER_TICKETS.title}>
