...
import {
  ...
  GET_SALE_PAGE_LIST_FOR_BOOKING,
} from '../../../DAL';
...
const BookingFilter = ({navigation, route}) => {
  ...
  const [simpleOptionModal, setSimpleOptionModal] = useState({
    isVisble: false,
    list: [],
    titleKey: '',
  });
  ...
  const onSubmitButtonPress = () => {
    navigation.navigate(routes.bookingList, {filters});
  };

  const closeOptionModal = () => {
    setOptionModal({isVisble: false, list: [], type: '', titleKey: ''});
  };
  const closeSimpleOptionModal = () => {
    setSimpleOptionModal({isVisble: false, list: [], titleKey: ''});
  };
  ...
  const filterTheList = (list, searchText) => {
    ...
    if (optionModal.titleKey == 'sale_page_title') {
      nlist = list
        .slice()
        .filter(x => !filters.sale_page.find(y => y._id == x._id));
  ...
  const getBookingsPagesFromServer = async () => {
    let res = await GET_SALE_PAGE_LIST_FOR_BOOKING({
      navigation,
      token,
  ...
  const selectedMembersView = () => {
    return (
      <View style={__styles.selectedMembersView}>
  ...
  return (
    <RootView title={STRINGS.BOOKING_FILTER.filter}>
      <KeyboardAwareScrollView
        ...
        <View style={{marginBottom: 10}}>
          <MyCheckBox
            ...
            title={STRINGS.BOOKING_FILTER.filterByBookingDates}
          />
        </View>
        ...
        <Collapsible collapsed={!filters?.filter_by_dates}>
          <Row>
            <Flex flex={1}>
              <MyTouchableInput
                label={STRINGS.BOOKING_FILTER.from}
            ...
            <Flex flex={1} ml={20}>
              <MyTouchableInput
                label={STRINGS.BOOKING_FILTER.to}
                icon={() => icons.calendar(colors.primary)}
                onPress={() =>
                  ref_calendar?.current?.openModal(
                    filters?.end_date,
        ...
        <MyTouchableInput
          ...
          onPress={() =>
            setOptionModal({
              ...
              type: STRINGS.BOOKING_FILTER.bookingPage,
            })
          }
        ...
        <MyTouchableInput
          label={STRINGS.BOOKING_FILTER.selectBookingStatus}
          onPress={() =>
            setOptionModal({
              isVisble: true,
              list: statusList,
              titleKey: 'title',
              type: STRINGS.BOOKING_FILTER.bookingStatus,
            })
          }
          ...
          onClearButtonPress={() =>
            setFilters({...filters, booking_status: null})
          }
        />

        <MyTouchableInput
          label={STRINGS.BOOKING_FILTER.sortBy}
          onPress={() =>
            setSimpleOptionModal({
              isVisble: true,
              list: sortList,
              titleKey: 'sort_title',
            })
        ...
        <Row style={{marginTop: 10}}>
          ...
          <MyClearButton
            ...
            title={STRINGS.BOOKING_FILTER.clearFilter}
