...
const ChangeStatusModal = forwardRef(
  ({navigation, token, onStatusChange}, ref) => {
    const [isVisible, setIsVisible] = useState(false);
    const [statusList, setStatusList] = useState([]);
    const [selectedStatus, setSelectedStatus] = useState(null);
    const [addNotes, setAddNotes] = useState(false);
    const [desc, setDesc] = useState(
      '<b>BIO:</b><span>    <br><b>GOAL:</b> <br><b>PARADIGM:</b> <br><b>SOLUTION:</b>',
    );
    ...
    const onSubmit = async () => {
      if (!selectedStatus) {
        showToast({
          body: STRINGS.CHANGE_STATUS_MODAL.selectBookingStatus,
          title: STRINGS.CHANGE_STATUS_MODAL.alert,
          type: 'info',
        });
      } else {
        let obj = {
    ...
    const changeStausFromServer = async obj => {
      ...
      if (res.code == 200) {
        showToast({
          body: res?.message,
          title: STRINGS.CHANGE_STATUS_MODAL.statusChanged,
          type: 'success',
        });
      ...
      <Modal
        ...
        <SafeAreaView style={styles.rootView}>
          <View style={styles.headerContainer}>
            <View>
              <MyText fontSize={18} type="medium">
                {STRINGS.CHANGE_STATUS_MODAL.changeBookingStatus}
              </MyText>
            </View>
          ...
          <View style={styles.rootInnerView}>
            <View style={styles.contentContainer}>
              <KeyboardAwareScrollView showsVerticalScrollIndicator={false}>
                <MyTouchableInput
                  label={STRINGS.CHANGE_STATUS_MODAL.bookingStatus}
                ...
                <View style={styles.radioRootView}>
                  <MyText isLabel>
                    {STRINGS.CHANGE_STATUS_MODAL.addToPersonalNotes}
                  </MyText>
                  <View style={styles.radioView}>
                    <View style={styles.radioItem}>
                      <MyCheckBox
                        title={STRINGS.CHANGE_STATUS_MODAL.yes}
                    ...
                    <View style={styles.radioItem}>
                      <MyCheckBox
                        title={STRINGS.CHANGE_STATUS_MODAL.no}
                ...
                <Editor
                  label={STRINGS.CHANGE_STATUS_MODAL.note}
                ...
                <MyButton
                  ...
                  invert
                  title={STRINGS.CHANGE_STATUS_MODAL.update}
                  onPress={onSubmit}
                />
          ...
          <OptionModalWithSearch
            ...
            title={STRINGS.CHANGE_STATUS_MODAL.status}
            onSelected={opt => {
              setOptionModalVisibility(false);
