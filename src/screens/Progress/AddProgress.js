...
const AddProgress = ({navigation, route}) => {
  ...
  const [AddType, setAddType] = useState(
    !!editableItem?.report_type
      ? typeList.find(x => x.key == editableItem?.report_type)
      : typeList[0],
  );
  const [startDate, setStartDate] = useState(
    !!editableItem?.start_date ? moment(editableItem?.start_date) : moment(),
  );
  const [endDate, setEndDate] = useState(
    !!editableItem?.end_date ? moment(editableItem?.end_date) : moment(),
  );
  const [category, setCategory] = useState(
    !!editableItem?.progress_category ? editableItem?.progress_category : null,
  );
  const [desc, setDesc] = useState(
    !!editableItem?.description ? editableItem?.description : '',
  );
  const [image, setImage] = useState(
    !!editableItem?.image ? editableItem?.image?.thumbnail_1 : null,
  );
  ...
  const updateToServer = async fd => {
    let res = await PROGRESS_UPDATE({
      navigation,
      token,
      formdata: fd,
      progressId: editableItem?._id,
    });
    setLoader(false);
    if (res.code == 200) {
      navigation.navigate(routes?.progresssList, {
    ...
    <RootView title={!!editableItem ? 'Edit Progress' : 'Add New Progress'}>
      <KeyboardAwareScrollView
        ...
        <MyTouchableInput
          label="Add Type *"
          onPress={() =>
            setOptionModal({isVisble: true, list: typeList, type: 'addtype'})
          }
        ...
        <MyTouchableInput
          label="Start Date *"
          onPress={() =>
            ref_calendar?.current?.openModal(startDate, 'startDate')
          }
          value={
            !!startDate ? moment(startDate).format(dateTimeFormat.date) : ''
          }
        ...
        <MyTouchableInput
          ...
          onPress={() =>
            setOptionModal({isVisble: true, list: catList, type: 'category'})
          }
