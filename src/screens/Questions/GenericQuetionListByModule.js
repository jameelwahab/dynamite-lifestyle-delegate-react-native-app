...
import { DELETE_QUESTIONS, QUESTIONS_LIST, QUESTIONS_LIST_BY_MODULE } from '../../DAL'
...
const GenericQuetionListByModule = ({ module, moduleId }) => {
  ...
  const deleteQuestionFromServer = async (item) => {
    let res = await DELETE_QUESTIONS({
      token, navigation, questionId: item?._id
    })
    if (res.code == 200) {
      setList((list) => {
  ...
  const onAddEditQuestion = (item) => {
    navigation.navigate(routes.addEditQuestions, {
      module, moduleId, screenName: route?.name, item
    });
  }
  ...
  return (
    <View style={{ flex: 1 }}>
      ...
      <ConfirmationModal
        ...
        closeModal={() => setConfirmModal({ isVisible: false, selectedItem: null })}
        onAgree={onAgree}
