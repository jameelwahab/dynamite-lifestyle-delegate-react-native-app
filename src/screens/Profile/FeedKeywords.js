...
const FeedKeywords = ({navigation}) => {
  ...
  const notifcationTypeHandler = (el, index) => {
    if (!isNotifyPresent(index, el)) {
      setFeedKeywordSetting(
        feedKeywordSetting.map((e, i) =>
          i == index
            ? {
                ...feedKeywordSetting[i],
                notifications: [
                  ...feedKeywordSetting[i].notifications,
                  {...el},
                ],
              }
            : e,
      ...
      setFeedKeywordSetting(
        feedKeywordSetting.map((e, i) =>
          i == index
            ? {
                ...feedKeywordSetting[i],
                notifications: feedKeywordSetting[i].notifications.filter(
                  e => e.name != el.name,
                ),
              }
            : e,
  ...
  const handleNotifyEdit = (el, index) => {
    if (el.name == 'email_notification_access') {
      ref_email_modal.current.openModal(
        feedKeywordSetting[index].notifications.find(e => e.name == el.name)
          .email_notification_info || {},
        index,
      );
    } else if (el.name == 'push_notification_access') {
      ref_notification_modal.current.openModal(
        feedKeywordSetting[index].notifications.find(e => e.name == el.name)
          .push_notification_info,
        index,
      );
    } else if (el.name == 'message_notification_access') {
      ref_message_modal.current.openModal(
        feedKeywordSetting[index].notifications.find(e => e.name == el.name)
  ...
  const isNotifyPresent = (indexFeed, el) => {
    return (
      feedKeywordSetting[indexFeed].notifications.findIndex(
        x => x.name == el.name,
      ) > -1
    );
  };
  ...
  const notifyDatHandler = (data, index, type) => {
    let iindex = feedKeywordSetting[index].notifications.findIndex(
      x => x.name == type,
    );
    if (iindex > -1) {
      setFeedKeywordSetting(
        feedKeywordSetting.map((el, ind) =>
          ind == index
            ? {
                ...feedKeywordSetting[ind],
                notifications: feedKeywordSetting[ind].notifications.map(
                  (e, i) => (e.name == type ? {...e, ...data} : e),
                ),
              }
            : el,
        ),
    ...
    setFeedKeywordSetting(
      feedKeywordSetting.map((el, ind) =>
        indexFeed == ind
          ? {
              ...feedKeywordSetting[ind],
              keywords: feedKeywordSetting[ind].keywords.filter(
                (x, i) => i != index && x,
              ),
            }
          : el,
  ...
  const ccView = index => {
    if (feedKeywordSetting[index].keywords.length > 0) {
      return (
        <View style={__styles.ccViewContainer}>
          {feedKeywordSetting[index].keywords.map((x, i) => (
            <MyChip
              title={x.value}
              key={i}
              onPress={() => removeCc(index, i)}
            />
          ))}
        </View>
      );
  ...
  const addAndRemo = (txt, index) => {
    if (txt == 'add') {
      ...
      setFeedKeywordSetting(
        feedKeywordSetting.filter((_, ind) => ind != index),
      );
  ...
  const updateTheList = async () => {
    ...
    const res = await UPDATE_FEED_KEYWORDS_SETTINGS({
      token,
      navigation,
      feed_keyword_setting: feedKeywordSetting,
    });
    if (res.code == 200) {
    ...
    return (
      <View style={__styles.notificationView}>
        <Row alignItems="center">
          <Flex flex={1}>
            <MyText color={colors.lightText2} type="medium">
              {STRINGS.FEED_KEYWORDS.keywords}
            </MyText>
          </Flex>
          <View style={__styles.btnContainer}>
            {feedKeywordSetting.length != 1 && (
              <TouchableOpacity
                onPress={() => addAndRemo('rm', index)}
                style={__styles.padding5}>
                {icons.minusCircle()}
              </TouchableOpacity>
            )}
        ...
        <View>
          <MyInputs
            ...
            rightIcon={
              cc[index] != '' ? () => icons.plus(colors.primary) : null
            }
            rightIconOnPress={() => {
              setFeedKeywordSetting(
                feedKeywordSetting.map((el, ind) =>
                  ind == index
                    ? {
                        ...feedKeywordSetting[ind],
                        keywords: [
                          ...feedKeywordSetting[ind].keywords,
                          {value: cc[index]},
                        ],
                      }
                    : el,
        ...
        {notify.map((el, i) => {
          ...
          return (
            <View style={__styles.notifyContainer} key={i}>
              ...
              <TransparentButton
                ...
                icon={() =>
                  icons.editpencil(
                    isPresent ? colors.primary : colors.transparent,
                  )
                }
              />
            </View>
  ...
  return (
    <RootView
      ...
      title={STRINGS.FEED_KEYWORDS.title}>
      <Flex flex={1}>
        <MyKeyboardAvoidingView noScrollView>
          <Flex flex={1}>
            <FlatList
              ...
              ListFooterComponentStyle={__styles.listFooterStyle}
        ...
        {isArray(feedKeywordSetting) && (
          <MyButton
            title={STRINGS.FEED_KEYWORDS.update}
            onPress={updateTheList}
          />
        )}
      </Flex>
...
const notify = [
  {
    label: STRINGS.FEED_KEYWORDS.notification,
    name: 'push_notification_access',
  },
  {
    label: STRINGS.FEED_KEYWORDS.message,
    name: 'message_notification_access',
  },
  {
    label: STRINGS.FEED_KEYWORDS.email,
