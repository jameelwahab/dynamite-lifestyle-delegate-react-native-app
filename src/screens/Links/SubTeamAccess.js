...
import { GET_LINKS_SALES_TEAM_LIST_FOR_ACCESS, UPDATE_LINKS_SALES_TEAM_ACCESS, } from '../../DAL'
...
const SubTeamAccess = ({ navigation, route }) => {
  ...
  const getDataFromServer = async () => {
    let res = await GET_LINKS_SALES_TEAM_LIST_FOR_ACCESS({ token, navigation, salePageId: _id });
    if (res.code == 200) {
  ...
  const updateSaleCommission = async (commission) => {
    ...
    let res = await UPDATE_LINKS_SALES_TEAM_ACCESS({
      token, navigation, salePageId: _id,
    ...
    return (
      <View style={__styles.itemView}>
        <View style={{ flexDirection: "row", alignItems: "center", paddingHorizontal: 5, }}>
          <View opacity={item?.is_approved ? 1 : 0.6} pointerEvents={item?.is_approved ? "auto" : "none"} >
            <MyCheckBox
              pb={0}
          ...
          <View style={[__styles.statusView, { backgroundColor: item?.is_approved ? colors.green : colors.heart }]}>
            <MyText fontSize={12} color={item?.is_approved ? colors.black : colors.text} >{item?.is_approved ? "Approved" : "Pending"}</MyText>
          </View>
  ...
  const footerView = () => {
    return (
      <View style={{ marginHorizontal: 5 }}>
        {list.length > 0 &&
          <MyButton onPress={updateSaleCommission} title='Save Changes' />}
      </View>
    )
  }
