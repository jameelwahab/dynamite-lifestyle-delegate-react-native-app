...
const ChatList = ({navigation}) => {
  ...
  const newMsgReceive = data => {
    if (data.code == 200) {
      setChatList(chatList => {
        let index = chatList.findIndex(
          x => x?._id == data?.chat_obj?.chat?._id,
        );
        if (index > -1) {
          let chatobj = {...chatList[index]};
          let memberIndex = chatobj.member.findIndex(
            x => x._id?._id != user?._id,
          );
          ...
          chatobj.member[memberIndex] = {
            ...
            badge_info: {
              color_code:
                chatList[index]?.member[memberIndex]?.badge_info?.color_code,
            },
          };
  ...
  const editMessageReceiver = data => {
    ...
    if (data.code == 200) {
      setChatList(chatList => {
        let index = chatList.findIndex(
          x => x?._id == data?.message_obj?.chat_id,
        );
        if (index > -1) {
          let chatobj = {...chatList[index]};
      ...
      <View style={__style.headerContainer}>
        <View style={__style.tabsView}>
          ...
          <TouchableOpacity
            ...
            style={[
              __style.tabView,
              tab == 'unread' && __style.tabSelectedView,
            ]}>
            <MyText color={tab == 'unread' ? colors.primary : undefined}>
              {STRINGS.CHAT_LIST.unread}
            </MyText>
          </TouchableOpacity>
        </View>
        ...
        <MyTouchableInput
          ...
          noSpace={true}
          value={eventId.title}
          label={STRINGS.CHAT_LIST.portals}
        />
        <MyInputs
          leftIcon={icons.search}
          placeholder={STRINGS.CHAT_LIST.searchPlaceholder}
          value={searchText}
    ...
    return (
      <TouchableHighlight
        ...
        <View style={__style.itemRootView}>
          <View style={__style.imageContainer}>
            <UserImage
              borderWidth={2}
            ...
            <View
              style={[
                __style.status,
                {
                  backgroundColor: member?._id?.is_online
                    ? colors.online
                    : colors.primary2,
                },
              ]}
          ...
          <View style={__style.seondViewRow}>
            ...
            <View style={__style.msg_view}>
              ...
              <View style={__style.msgContainer}>
                <MyText
                  fontSize={12}
                  type="light"
                  numberOfLines={1}
                  style={__style.msgText}>
                  {!!item?.last_message
                    ...
                    : item?.message_type == 'image'
                    ? STRINGS.CHAT_LIST.photo
                    : item?.message_type == 'audio'
                    ? STRINGS.CHAT_LIST.audio
                    : item?.message_type == 'video'
                    ? STRINGS.CHAT_LIST.video
                    : ''}
                </MyText>
  ...
  return (
    <RootView hideBackBottomButton title={STRINGS.CHAT_LIST.title} hideChatIcon>
      <View style={__style.flex1}>
        <FlatList
          data={chatList}
          renderItem={renderChatList}
          ListEmptyComponent={
            !loader && <EmptyView label={STRINGS.CHAT_LIST.noChat} />
          }
          ListHeaderComponent={headerView()}
          stickyHeaderIndices={[0]}
          stickyHeaderHiddenOnScroll={true}
          contentContainerStyle={__style.flatListContent}
      ...
      {isChatAllowed && (
        <FAB
          onPress={() =>
            navigation.navigate(routes.startNewChat, {
              resetCountToZero,
              refresh,
            })
          }
