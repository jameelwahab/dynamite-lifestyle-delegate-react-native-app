...
import { ADD_NOTES, ASSESSMENT_NOTE_ADD, ASSESSMENT_NOTE_UPDATE, EDIT_NOTES } from '../../../DAL'
...
const AddNote = ({ navigation, route }) => {
  ...
  const [note, setNote] = useState(!!oldNote ? oldNote?.internal_note_message : "");
  ...
  const addTheNote = async () => {
    ...
    if (!!oldNote) {
      res = await ASSESSMENT_NOTE_UPDATE({
        token, navigation, assessmentId, type, noteId: oldNote?._id, message: note
      });
    } else {
      res = await ASSESSMENT_NOTE_ADD({
        token, navigation, assessmentId, type, message: note
      });
    }
    setLoader(false)
    if (res.code == 200) {
