...
const NotificationList = ({navigation, route}) => {
  ...
  const onNextScreen = item => {
    ...
    if (feedType.includes(notification_type)) {
      let navigator = '';
      if (
        (item?.tab_type == 'the_cosmos' ||
          item?.notification_type == 'feed_mentioned') &&
        !!navbar.find(x => x.value == 'the_cosmos')
      ) {
      ...
      if (!!navigator) {
        ...
        if (
          notification_type == 'addcomment' ||
          notification_type == 'addcommentreply' ||
          notification_type == 'commentlike' ||
          notification_type == 'feed_comment_mentioned'
        ) {
      ...
      if (
        item?.support_ticket?.action_by == 'member_user' &&
        !!navbar.find(x => x.value == 'support_ticket')
      ) {
        ...
        isMineTicket &&
        !!navbar
          .find(x => x.value == 'support')
          ?.child_options.find(x => x.value == 'contact_support')
      ) {
        ...
        !isMineTicket &&
        !!navbar.find(x => x.value == 'internal-tickets')
      ) {
      ...
      !!navbar.find(x => x.value == 'delegate_events')
    ) {
      navigation.reset({
        routes: [
          {
  ...
  const getNotificationListFromServer = async (firstTime = false) => {
    ...
    if (res.code == 200) {
      let listLength = firstTime
        ? 0 + res.notification_array.length
        : list.length + res.notification_array.length;
      ...
      setList(
        firstTime
          ? res.notification_array
          : [...list, ...res.notification_array],
      );
  ...
  const deleteSingal = async id => {
    ...
    if (res.code == 200) {
      showToast({
        title: 'Notification has been deleted successfully',
        type: 'success',
      });
      let index = list.findIndex(x => x._id == id);
      let temp_total = total;
  ...
  const deleteAllNotification = async () => {
    ...
    if (res.code == 200) {
      showToast({
        title: 'All Notifications has been deleted successfully',
        type: 'success',
      });
      setList([]);
      setTotal(0);
  ...
  const renderItem = ({item, index}) => {
    return (
      <TouchableOpacity
        ...
        style={[
          __styles.rootItemView,
          !item?.is_seen && __styles.rootItemUnseenView,
        ]}>
        <View
          style={[
            __styles.dot,
    ...
    return (
      <View style={{flexDirection: 'row'}}>
        ...
        {list.length > 0 && (
          <>
            {total > 0 && (
              <TouchableOpacity onPress={markAllRead} style={__styles.btn}>
    ...
    <RootView titleView={topView} hideNotificaitonIcon hideChatIcon>
      <View style={{flex: 1}}>
        <FlatList
          ...
          ListEmptyComponent={
            !loader &&
            !refreshing && <EmptyView label={'No Notifications Found'} />
          }
          refreshControl={
