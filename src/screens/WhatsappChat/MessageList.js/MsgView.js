    ...
    if (!!template) {
      ...
      return (
        <View>
          {!!header &&
            <View>
              {header?.format == "IMAGE" ? templateImage(header?.example?.header_handle[0]) :
                <View style={{ marginBottom: 5 }}>
                  <MyText color={isMe1 ? colors.white : colors.black} fontSize={14} type='bold' >{header?.text}</MyText>
                </View>}
            </View>}
