...
const RecordingAddEdit = ({navigation, route}) => {
  ...
  const [data, updateData] = useState({
    ...
    date: !!item?.recording_date
      ? moment(item?.recording_date, 'DD MMM, YYYY')
      : moment(),
    ...
    image: !!item?.recording_image?.thumbnail_1
      ? item?.recording_image?.thumbnail_1
      : '',
  ...
  const onSubmitPress = () => {
    if (!isArray(missions) && !category && !isArray(programme)) {
      showToast({
        body: 'Please select at least one of the mission/quest, program, or vault category.',
        title: 'Error',
        type: 'error',
      });
      setLoader(false);
    ...
    if (isArray(programme))
      fd.append(
        'programs',
        isArray(programme) ? JSON.stringify(programme) : '',
      );
  ...
  const removeAudioFromServer = async () => {
    setLoader(true);
    let res = await RECORDING_REMOVE_AUDIO({
      navigation,
      token,
  ...
  const upadte_recording_to_server = async fd => {
    setLoader(true);
    let res = await RECORDING_UPDATE({
      navigation,
      token,
      formdata: fd,
      slug: item?.recording_slug,
        ...
        <KeyboardAwareScrollView
          ...
          <MyTouchableInput
            ...
            iconOnPress={() =>
              setOptionModal({
                isVisible: true,
                list: isArray(programme)
                  ? programmes.filter(
                      el => programme.findIndex(i => i._id == el._id) < 0,
                    )
                  : programmes,
                type: 'programme',
                key: 'title',
          ...
          <MyTouchableInput
            ...
            iconOnPress={() =>
              setOptionModal({
                isVisible: true,
                list: isArray(missions)
                  ? listMission.filter(
                      el => missions.findIndex(i => i._id == el._id) < 0,
                    )
                  : listMission,
                type: 'missions',
                key: 'title',
                  ...
                  missions?.map((el, index) => (
                    <View
                      ...
                      <MyChip
                        ...
                        onPress={() => {
                          setData({
                            missions: missions.filter(ele => ele._id != el._id),
                          });
                        }}
                      />
                    </View>
      ...
      <OptionModal
        ...
        onSelected={opt => {
          if (optionModal.type == 'programme') {
            setData({
              programme: isArray(programme) ? [...programme, opt] : [opt],
            });
