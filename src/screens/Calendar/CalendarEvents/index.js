...
const CalendarScreen = ({navigation, route}) => {
  ...
  const [title] = useState(
    isDelegateEvents
      ...
      : navbar
          ?.find(x => x._id == parentKey)
          ?.child_options?.find(y => y._id == key)?.title,
  );
  ...
  const getCalendarEventsLists = async () => {
    ...
    let res = await GET_CALENDAR_EVENTS_LIST({
      navigation,
      token,
      body: {created_for: eventType, end_date, start_date},
    });
    ...
    if (res.code == 200) {
      ...
      for (let i = 0; i < diff; i++) {
        ...
        let list = res.event.filter(
          x =>
            convertTimezone2(x?.start_date_time, timezone).format(
              dateStringCalendar,
            ) == date,
        );
  ...
  const onBackPress = () => {
    setCurDate(date =>
      moment(date).subtract(type, 1).format(dateStringCalendar),
    );
  };
    ...
    return (
      <>
        ...
        <View style={__styles.topView}>
          <View style={__styles.flexEndContainer}>
      ...
      <View style={__styles.calendarView}>
        <Calendar
          ...
          dayComponent={({date, state}) => {
            let textColor =
              curDate == date.dateString
                ? colors.black
                : state == 'today'
                ? colors.primary
                : colors.white;
            let bgColor =
              curDate == date.dateString ? colors.primary : colors.transparent;
            return (
              <TouchableOpacity
                ...
                {calendarEvents[date?.dateString]?.count > 0 && (
                  <MyText color={textColor} fontSize={10}>
                    {calendarEvents[date?.dateString]?.count}{' '}
                  </MyText>
                )}
              </TouchableOpacity>
            );
          ...
          renderHeader={date => (
            <MyText fontSize={16} type="medium">
              {moment(curDate).format(STRINGS.CALENDAR_EVENTS.dateFormat)}
            </MyText>
          )}
        />
      </View>
  ...
  const subHeadingView = () => {
    return (
      <View style={__styles.headingRow}>
        <TransparentButton onPress={onBackPress} icon={icons.backwardArrow} />
        <View style={__styles.headingText}>
          <MyText color={colors.primary} type="medium" fontSize={16}>
            {type == 'week'
              ? moment(curDate).startOf('week').format(dateTimeFormat.date) +
                '  -  ' +
                moment(curDate).endOf('week').format(dateTimeFormat.date)
              : moment(curDate).format(dateTimeFormat.date)}
          </MyText>
          {/* <MyText>{moment(curDate).startOf("week").format("dddd") + "  -  " + moment(curDate).endOf("week").format("dddd")}</MyText> */}
        </View>
        <TransparentButton onPress={onNextPress} icon={icons.forwardArrow} />
      </View>
    );
  };
    ...
    if (type == 'week') {
        ...
        <View style={[__styles.eventView]}>
          <View style={__styles.weekDayView}>
            ...
            <MyText color={colors.primary}>
              {moment(item).format('MM') + '/' + moment(item).format('DD')}
            </MyText>
          </View>
                ...
                <View style={__styles.eventTitleAndDetail}>
                  ...
                  <View style={__styles.timeView}>
                    <MyText fontSize={10}>
                      ...
                      -{' '}
                      {convertTimezone2(event?.end_date_time, timezone).format(
                        dateTimeFormat.time,
                      )}
                    </MyText>
                  </View>
      ...
      return (
        <TouchableOpacity
          ...
          <View
            style={[__styles.eventColorView, {backgroundColor: item?.color}]}
          />
          <View style={__styles.eventTitleAndDetail}>
            <MyText type="medium">{item?.title}</MyText>
            <View style={__styles.timeView}>
              <MyText fontSize={10}>
                {convertTimezone2(item?.start_date_time, timezone).format(
                  dateTimeFormat.time,
                )}{' '}
                -{' '}
                {convertTimezone2(item?.end_date_time, timezone).format(
                  dateTimeFormat.time,
                )}
              </MyText>
            </View>
      ...
      <View style={__styles.eventViewContainer}>
        {list.map((x, i) => {
          if ((i >= 3 && seeMore[id]) || i < 3)
            return <MyText key={x?._id?._id}>{x?._id?.title},</MyText>;
        })}
        {count > 3 && (
          <TouchableOpacity
            onPress={() => toggleSee(id)}
            hitSlop={__styles.seeMoreHitSlop}>
            <MyText underlined color={colors.primary}>
              {seeMore[id] ? STRINGS.CALENDAR_EVENTS.seeLess : STRINGS.CALENDAR_EVENTS.seeMore}
            </MyText>
          </TouchableOpacity>
        )}
      </View>
    ...
    return (
      <View
        ...
        <MyText
          ...
          {value ? STRINGS.CALENDAR_EVENTS.active : STRINGS.CALENDAR_EVENTS.inactive}
        </MyText>
      </View>
    );
  };
  ...
  const renderEventItem = ({item, index}) => {
    const handlePress = () =>
      navigation.navigate(routes.calendarGroupEventDetail, {item});
    return (
      <Pressable onPress={handlePress} style={__styles.eventListView}>
        <View style={__styles.eventListTitleRow}>
          <View style={__styles.flex1Container}>
            <MyText type="medium">{item?.title}</MyText>
          </View>
          <View style={__styles.menuButtonOffset}>
            <MenuButton onPress={() => setOptions({isVisible: true, item})} />
          </View>
        </View>
        <View>
          <StatView title={STRINGS.CALENDAR_EVENTS.groups} view={() => eventView(item?.group)} />
          <StatView title={STRINGS.CALENDAR_EVENTS.eventType} value={item?.recurring_type} />
          <StatView
            title={STRINGS.CALENDAR_EVENTS.startDate}
            value={convertTimezone2(item?.start_date, timezone).format(
              dateTimeFormat.date,
            )}
          />
          <StatView
            title={STRINGS.CALENDAR_EVENTS.endDate}
            value={convertTimezone2(item?.end_date, timezone).format(
              dateTimeFormat.date,
            )}
          />
          <StatView title={STRINGS.CALENDAR_EVENTS.members} value={item?.member.length} />
          <StatView
            title={STRINGS.CALENDAR_EVENTS.status}
            view={() => statusView(item?.status, item?._id)}
          />
        </View>
      </Pressable>
    );
  ...
  const titleView = () => {
    return (
      <View style={__styles.titleViewContainer}>
        <TitleView hideBackBottomButton title={title} />
        <View style={__styles.flexEndContainer}>
          {!isDelegateEvents && (
            <TouchableOpacity
              onPress={() =>
                setViewType(type => (type == 'calendar' ? 'list' : 'calendar'))
              }
              style={__styles.toggleBtnView}>
              {viewType == 'calendar'
                ? icons.list(colors.black, 18)
    ...
    <RootView hideBackBottomButton titleView={titleView}>
      {viewType == 'calendar' ? (
        <FlatList
          ...
          ListEmptyComponent={
            !loader && <EmptyView label={STRINGS.CALENDAR_EVENTS.noEventsFound} />
          }
        />
      ) : (
        <FlatList
      ...
      <ConfirmationModal
        title={STRINGS.CALENDAR_EVENTS.deleteConfirmation}
...
const optionsList = [
  {
    title: STRINGS.CALENDAR_EVENTS.edit,
    key: 'edit',
    icon: icons.edit,
  },
  {
    title: STRINGS.CALENDAR_EVENTS.delete,
    key: 'delete',
    icon: icons.trash,
  },
  // {
