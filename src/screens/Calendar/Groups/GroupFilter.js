...
const GroupFilter = ({navigation, route}) => {
  ...
  const filterBadgeList = list => {
    if (!!list && badges.length == list.length) {
      return [{title: STRINGS.CALENDAR_GROUP_FILTER.noOptions, key: 'no'}];
    } else if (badges.length == 0) {
      return list;
    } else {
      return list.filter(
  ...
  const filterGroupList = () => {
    if (groupList.length == 0) {
      ...
      return list.data.filter(
        el => groupList.findIndex(x => x._id == el._id) < 0 && el,
      );
    }
  };
  const filterTheList = (list, text) => {
    if (text.trim() == '') {
      ...
      return list
        .slice()
        .filter(x =>
          x[groupBy.key == 'sale_page' ? 'sale_page_title' : 'title']
            .toLowerCase()
      ...
      <View style={__styles.viewContainer}>
        {groupList.map((el, index) => (
          <View style={__styles.viewCards} key={index}>
            <MyChip
              title={groupBy.key == 'sale_page' ? el.sale_page_title : el.title}
              index={index}
              onPress={() => {
  ...
  return (
    <RootView title={STRINGS.CALENDAR_GROUP_FILTER.title}>
      <MyTouchableInput
        label={STRINGS.CALENDAR_GROUP_FILTER.groupBy}
        value={groupBy?.title || ''}
        icon={() => icons.down()}
      ...
      <MyTouchableInput
        label={STRINGS.CALENDAR_GROUP_FILTER.badgeLevel}
        value={''}
      ...
      <View style={__styles.btn_container}>
        <MyClearButton
          style={__styles.clearButton}
          title={STRINGS.CALENDAR_GROUP_FILTER.clearFilter}
          onPress={() => {
            navigation.navigate(routes.calendarGroupList, {filter: {}});
          }}
        />
        <MyButton
          style={__styles.submitButton}
          title={STRINGS.CALENDAR_GROUP_FILTER.submit}
      ...
      <OptionModal2
        ...
        multipleLabel={STRINGS.CALENDAR_GROUP_FILTER.selectBadgeLevels}
      />
