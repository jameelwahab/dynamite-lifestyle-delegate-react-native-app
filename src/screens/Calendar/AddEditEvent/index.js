...
const GroupAddEdit = ({navigation, route}) => {
  ...
  const [groupData, updateGroupData] = useState({
    ...
    member: !!route?.params?.member
      ? [route?.params?.member]
      : !!event?.member
      ? event?.member.map(x => x?._id)
      : [],
    exclude_members: !!route?.params?.event?.exclude_members
      ? route?.params?.event?.exclude_members.map(x => x?._id)
      : !!event?.excluded_members
      ? event?.excluded_members.map(x => x)
      : [],
    startDate: !!event?.start_date_time
      ? convertTimezone2(event?.start_date_time, timezone)
      : moment(),
    startTime: !!event?.start_date_time
      ? convertTimezone2(event?.start_date_time, timezone).format('HH:mm')
      : moment().format('HH:mm'),
    endDate: !!event?.end_date_time
      ? convertTimezone2(event?.end_date_time, timezone)
      : moment(),
    endTime: !!event?.end_date_time
      ? convertTimezone2(event?.end_date_time, timezone).format('HH:mm')
      : moment().format('HH:mm'),
  ...
  const getMemberListFromServer = async (searchText = '') => {
    let res = await GET_GROUPS_AND_MEMBERS_FOR_CALENDAR({
      navigation,
      token,
  ...
  return (
    <RootView
      title={
        isEditIteration ? 'Edit Iteration' : isEdit ? 'Edit Event' : 'Add Event'
      }>
      <MyKeyboardAvoidingView>
        ...
        <MyTouchableInput
          ...
          view={() => (
            <View
              style={[__styles.colorView, {backgroundColor: groupData.color}]}
            />
          )}
        />
          ...
          <View>
            ...
            <View style={__styles.alignBtnsRow}>
              <Pressable
                ...
                style={[
                  __styles.alignBtnView,
                  groupData?.recurringType == 'daily' &&
                    __styles.alignSelectedBtnView,
                ]}>
                <MyText
                  type="medium"
                  color={
              ...
              <Pressable
                ...
                style={[
                  __styles.alignBtnView,
                  groupData.recurringType == 'weekly' &&
                    __styles.alignSelectedBtnView,
                ]}>
                <MyText
                  type="medium"
              ...
              <Pressable
                ...
                style={[
                  __styles.alignBtnView,
                  groupData.recurringType == 'monthly' &&
                    __styles.alignSelectedBtnView,
                ]}>
                <MyText
                  type="medium"
        ...
        <View style={{flexDirection: 'row'}}>
          ...
          <View style={{flex: 1, marginRight: 10}}>
            <MyTouchableInput
              ...
              icon={icons.clock}
              value={moment(groupData?.startTime, 'HH:mm').format(
                dateTimeFormat.time,
              )}
            />
          </View>
        ...
        <View style={{flexDirection: 'row'}}>
          <View style={{flex: 1, marginRight: 10}}>
            <MyTouchableInput
              label="End Date*"
              onPress={() =>
                ref_calendar?.current?.openModal(groupData.endDate, 'endDate')
              }
              icon={() => icons.calendar(colors.primary)}
          ...
          <View style={{flex: 1, marginRight: 10}}>
            <MyTouchableInput
              label="End Time*"
              onPress={() =>
                ref_timePicker?.current?.openModal(groupData.endTime, 'endTime')
              }
              icon={icons.clock}
              value={moment(groupData?.endTime, 'HH:mm').format(
                dateTimeFormat.time,
              )}
            />
          </View>
        ...
        {!isEditIteration && !isDelegateEvents && (
          <>
            <MyTouchableInput
              ...
              iconOnPress={() =>
                setOptionModal({isVisible: true, type: 'group'})
              }
            />
            ...
            <MyTouchableInput
              ...
              label="Members"
              iconOnPress={() =>
                setOptionModal({isVisible: true, type: 'member'})
              }
            />
        ...
        <View style={{marginTop: 10}}>
          <MyButton title="Next" onPress={onSubmit} />
        </View>
      </MyKeyboardAvoidingView>
