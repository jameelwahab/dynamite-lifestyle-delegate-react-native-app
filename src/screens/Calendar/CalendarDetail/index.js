...
const CalendarDetail = ({navigation, route}) => {
  ...
  const getCalendarEventDetail = async () => {
    setLoader(true);
    let res = await CALENDAR_EVENT_DETAIL_BY_Id({
      navigation,
      token,
      id: eventId,
    });
  ...
  const titleView = () => (
    <View style={__styles.headerView}>
      <View style={{flex: 1}}>
        <TitleView title={data?.title} hideBackBottomButton />
      </View>
          ...
          {!!data && (
            <View style={{flex: 1}}>
              <View style={__styles.rootView}>
                ...
                <StatView
                  title={'To'}
                  value={convertTimezone2(data?.end_date_time, timezone).format(
