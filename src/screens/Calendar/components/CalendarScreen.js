...
const CalendarScreen = ({ navigation, route }) => {
  ...
  const getCalendarEventsLists = async () => {
    ...
    let res = await GET_CALENDAR_EVENTS_LIST({
      navigation, token, body: { created_for: "", end_date, start_date, }
    });
    ...
    if (res.code == 200) {
      ...
      for (let i = 0; i < diff; i++) {
        ...
        let list = res.event.filter(x => convertTimezone(x?.start_date_time, timezone).format(dateStringCalendar) == date);
  ...
  const deleteCalendarEvents = async (item) => {
    setLoader(true);
    let res = await GET_ALL_CALENDAR_EVENTS_LIST({ navigation, token, });
    if (res.code == 200) {
      setEventList((list)=>list.spice().filter(x =>x._id != item._id));
      setLoader(false);
    } else {
      setLoader(false);
    }
        ...
        <View style={[__styles.eventView,]}>
          ...
          <View style={__styles.eventWeekView} >
            {calendarEvents[item]?.list.map((event,) => (
              <View style={__styles.eventWeekRootView}>
                ...
                <View style={__styles.eventTitleAndDetail}>
                  ...
                  <View style={__styles.timeView}>
                    <MyText fontSize={10} >{convertTimezone(event?.start_date_time, timezone).format(dateTimeFormat.time)}  -  {convertTimezone(event?.end_date_time, timezone).format(dateTimeFormat.time)}</MyText>
                  </View>
                </View>
              </View>
            ))}
      ...
      return (
        <View style={__styles.eventView}>
          ...
          <View style={__styles.eventTitleAndDetail}>
            ...
            <View style={__styles.timeView}>
              <MyText fontSize={10} >{convertTimezone(item?.start_date_time, timezone).format(dateTimeFormat.time)}  -  {convertTimezone(item?.end_date_time, timezone).format(dateTimeFormat.time)}</MyText>
            </View>
          </View>
        </View>
      )
    ...
    return (
      <View style={__styles.eventListView}>
        ...
        <View>
          ...
          <StatView title={"Start Date"} value={convertTimezone(item?.start_date, timezone).format(dateTimeFormat.date)} />
          <StatView title={"End Date"} value={convertTimezone(item?.end_date, timezone).format(dateTimeFormat.date)} />
