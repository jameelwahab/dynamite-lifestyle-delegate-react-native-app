...
const LikeModalForComments = forwardRef(
  ({navigation, token, timezone, user, onMessagePress, isChatAllowed}, ref) => {
    ...
    const getLikes = async () => {
      ...
      let res = await GET_COMMENT_LIKES_LIST({
        navigation,
        token,
        body: fd,
        page: page,
    ...
    const userLikeView = ({item, index}) => {
      return (
        <View style={__style.likeItemContainer}>
          <View>
            <UserImage
              borderWidth={2}
              borderColor={
