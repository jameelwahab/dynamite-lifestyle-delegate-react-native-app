...
import {
  CREATE_FEED,
  FEED_DETAIL,
  GET_DELEGATES_LIST_FROM_SERVER_FOR_MENTION_V1,
  GET_KEYWORDS_ADDED_BY_USER,
  UPDATE_FEED,
  UPLOAD_FEED_IMAGES,
} from '../../../DAL';
...
const AddPost = forwardRef(
  (
    ...
    const [feedTypeModalVisibility, setFeedTypeModalVisibility] =
      useState(false);
    ...
    const [postCeatedFor, setPostCreatedFor] = useState(
      isCosmos
        ? feedLevel != 'all'
          ? feedLevel
          : user?.team_type
        : PostCretedForSourceFeed[0].type,
    );
    const [postCeatedForArray, setPostCreatedForArray] = useState([
      PostCretedForSourceFeed[0],
    ]);
    ...
    const [publishDate, setPublishDate] = useState(
      moment().format(dateTimeFormat.date),
    );
    const [publishTime, setPublishTime] = useState('12:00 AM');
    const [eventModalVisible, setEventModalVisible] = useState(false);
    const [multipleLevelModalVisiblity, setMultipleLevelModalVisiblity] =
      useState(false);
      ...
      if (mentionList.length > 0) {
        ...
        let index = list.findIndex(
          x =>
            ...
            cursorPosition < x?.offset + x?.length,
        );
        if (index > -1) {
          list.splice(index, 1);
        }
    ...
    const textHandler = text => {
      if (
        text.trim() == '' ||
        (text.trim().length == 1 && mentionList.length > 0)
      ) {
        setMentionList([]);
      }
    ...
    const onPressOnMentions = obj => {
      ...
      mentionList.forEach(item => {
        if (_at_index < item.offset) {
          item.offset =
            item.offset +
            (`${obj?.first_name} ${obj?.last_name}`.trim().length - diff);
        }
      });
    ...
    const selectItemForEdit = item => {
      ...
      // if (isCosmos) {
      setPostCreatedFor(
        item?.created_for_level_or_type == 'both'
          ? 'delegate'
          : item?.created_for_level_or_type,
      );
      // }
      setPostCreatedForArray([
        PostCretedForSourceFeed.find(
          x => x.type == item?.created_for_level_or_type,
        ),
      ]);
      ...
      if (isArray(item?.mentioned_users) || isArray(item?.feed_keywords)) {
        let mUser = isArray(item?.mentioned_users)
          ? item?.mentioned_users.map(item => ({...item, type: 'mention'}))
          : [];
        let keywords = isArray(item?.feed_keywords)
          ? item?.feed_keywords.map(item => ({...item, type: 'keyword'}))
          : [];
      ...
      if (!!item?.event_info && Object.keys(item?.event_info).length > 1) {
        let alignment = !!item?.event_info?.button_alignment
          ? item?.event_info?.button_alignment
          : 'center';
      ...
      if (!!item?.schedule_date_time && !item?.is_publish) {
        setPublishDate(
          moment(item?.schedule_date_time)
            .tz(timezone.admin)
    ...
    const filterTheBadgeList = list => {
      if (!!list && badges.length == list.length) {
        ...
        return list.filter(
          el => badges.findIndex(x => x._id == el._id) < 0 && el,
        );
      }
    };
    ...
    const onBadgeSelect = opt => {
      if (
        access?.multiple_levels_in_source_all_source_scadule_feeds ||
        showBadges.list.length != access.badge_levels.length
      ) {
    ...
    const addPostBtn = async () => {
      ...
      if (
        ...
        showToast({
          body: 'Please add data to be posted',
          title: 'Alert',
          type: 'info',
        });
        return;
        ...
        let time = moment(
          moment(pollData?.expiryDate).format('YYYY-MM-DD') +
            ' ' +
            moment(pollData?.expiryTime).format('HH:mm'),
          'YYYY-MM-DD HH:mm',
        ).format('YYYY-MM-DD HH:mm');
        let time2 = convertTimezoneToRegion(moment(), timezone).format(
          'YYYY-MM-DD HH:mm',
        );
        ...
        if (isbefore) {
          showToast({
            title: 'Alert',
            body: 'Past time selection is not allowed. Please choose a future time.',
            type: 'info',
          });
          ...
          showToast({
            title: 'Alert',
            body: 'All poll option must be unique',
            type: 'info',
          });
        ...
        if (isbefore) {
          ...
          for (let i = 0; i < pollData?.options.length; i++) {
            ...
            if (obj?.question_statement.trim() == '') {
              showToast({
                body: `Please add Statement in Question ${index + 1}`,
                title: 'Alert',
                type: 'info',
              });
              return;
              ...
              showToast({
                body: `Please add all options in Question ${index + 1}`,
                title: 'Alert',
                type: 'info',
              });
              return;
              ...
              showToast({
                body: `All poll option must be unique in Question ${index + 1}`,
                title: 'Alert',
                type: 'info',
              });
              return;
      ...
      if ((notifyTxt.state == '' || notifyTxt.desc == '') && notifyUser) {
        return showToast({
          title: 'info',
          body: 'Notication statement and Notication description should be not be empty',
        });
      }
        ...
        images.forEach(image => {
          if (!!image?.uri) {
            ...
            uploadImageArray.push(
              UPLOAD_FEED_IMAGES({token, navigation, formData: ifd}),
            );
          } else {
      ...
      fd.append(
        'feed_images',
        postType == 'image' ? JSON.stringify(uploadedImages) : '[]',
      );
      fd.append(
        'mentioned_users',
        JSON.stringify(
          breakReference(mentionList)
            .filter(user => user?.type == 'mention')
            .map(user => {
              delete user['type'];
              return user;
            }),
      ...
      if (isCosmos) {
        if (!!editId) {
          ...
          fd.append(
            ...
            isCosmos ? postCeatedFor : JSON.stringify([postCeatedFor]),
          );
        }
      }
      ...
      if (!isCosmos && !isNoteMainFeed) {
        if (badges.length == 0 && badgeFor.key != 'all') {
          showToast({
            body: 'Please Specify a badge level',
            title: 'Alert',
            type: 'info',
          });
          setLoader(false);
        ...
        if (badgeFor.key == 'all') {
          ...
          fd.append('show_feed_to', badgeFor.key);
          fd.append(
            'feed_badge_level_ids',
            JSON.stringify(badges.map(x => x?._id)),
          );
        }
      }
      ...
      if (isScheduledFeed) {
        fd.append(
          'schedule_date_time',
          moment(publishDate, dateTimeFormat.date).format('YYYY-MM-DD') +
            ' ' +
            moment(publishTime, dateTimeFormat.time).format('HH:mm'),
        );
      }
      if (!!!editId) {
    ...
    const editTheFeedPostAPI = async fd => {
      ...
      if (res.code == 200) {
        ...
        if (res1.code == 200) {
          ...
          if (
            feedLevel != 'all' &&
            editFeed?.created_for_level_or_type !=
              res1.feeds.created_for_level_or_type &&
            !isCosmos
          ) {
    ...
    const getTheDelegateListFromServer = async text => {
      ...
      let res = await GET_DELEGATES_LIST_FROM_SERVER_FOR_MENTION_V1({
        ...
        data: {
          search_text: text,
          community_levels: isNoteMainFeed
            ? undefined
            : isCosmos
            ? [postCeatedFor]
            : badges.map(el => el?._id),
          ...
          allow_all_option_in_mention_feed:
            !isCosmos && !isNoteMainFeed
              ? access.allow_all_option_in_mention_feed
              : undefined,
        },
      });
    ...
    const EventModal = () => {
      ...
      const btn_addEvent = () => {
        if (title.trim() == '') {
          ...
          showToast({
            body: 'Please enter event button title',
            title: 'Alert',
            type: 'info',
          });
          return;
          ...
          showToast({
            body: 'Please enter event button link',
            title: 'Alert',
            type: 'info',
          });
          return;
                      ...
                      <View>
                        ...
                        <View style={__style.alignBtnsRow}>
                          <Pressable
                            ...
                            style={[
                              __style.alignBtnView,
                              buttonAlignment == 'left' &&
                                __style.alignSelectedBtnView,
                            ]}>
                            <MyText
                              type="medium"
                              color={
      ...
      mentionList.forEach(user => {
        ...
        parts.push(
          <Text
            style={
              user?.type == 'keyword'
                ? __style.keywordHighlightedText
                : __style.mentionUserText
            }>
            {
              // user?.type == "keyword" ? "#" : "" +
            ...
            <View
                  ...
                  <View style={{marginLeft: 10, flex: 1}}>
                    ...
                    <View style={__style.modalActionButtonRow}>
                      ...
                      {!!!editId && isScheduledFeed && (
                        <View opacity={0.7}>
                          <TouchableOpacity style={__style.modalDropBtns}>
                            <MyText>
                              {isScheduledFeed ? 'Schedule' : 'Publish'}
                            </MyText>
                          </TouchableOpacity>
                        </View>
                      )}
              ...
              /> : */}
                <>
                  ...
                  {isMentionListVisible &&
                    (delegateList.length > 0 || isMentionListLoading) && (
                      <View
                        style={{
                                ...
                                <TouchableOpacity
                                  ...
                                  <MemberView
                                    secondText={
                                      !isNoteMainFeed
                                        ? isCosmos
                                          ? makeCosmosLevel(item?.team_type)
                                          : ` ${
                                              !!item
                                                ?.membership_level_badge_info
                                                ?.membership_level_badge_title
                                                ? '(' +
                                                  item
                                                    ?.membership_level_badge_info
                                                    ?.membership_level_badge_title +
                                    ...
                                    customImage={
                                      ...
                                      item.last_name != ''
                                        ...
                                        : icons.people(colors.primary, 17)
                                    }
                                    hideEmail
                                  />
                                </TouchableOpacity>
                  ...
                  {isKeywordListVisible &&
                    (keywordList.length > 0 || isKeywordListLoading) && (
                      <View
                        style={{
                          paddingHorizontal: 15,
                          marginTop: 10,
                  ...
                  <View style={{paddingHorizontal: 10}}>
                          ...
                          <View>
                            <MyImage
                              source={{
                                uri: !!image.uri
                                  ? image.uri
                                  : S3_URL + image.thumbnail_1,
                              }}
                              style={{
                                width: (utilities.screenWidth() - 40) / 4,
                                aspectRatio: 1,
                                borderRadius: 10,
                                marginRight: 10,
                                overflow: 'hidden',
                            ...
                            <TouchableOpacity
                              onPress={() => {
                                setImages(images =>
                                  images.filter((x, i) => i != index),
                                );
                              }}
                              style={[
                                __style.inputCrossBtn,
                                {
                    ...
                    <View>
                      ...
                      <TouchableOpacity
                        ...
                        style={[
                          __style.inputCrossBtn,
                          {top: -5, backgroundColor: colors.black},
                        ]}>
                ...
                <View style={[__style.typeButtonRow, {paddingHorizontal: 10}]}>
                  <View style={{flex: 1, flexDirection: 'row'}}>
                    <TouchableOpacity
                      ...
                      {icons.camera(
                        postType == 'image' ? colors.primary : colors.white,
                        17,
                      )}
                    </TouchableOpacity>
                    ...
                    <TouchableOpacity
                      ...
                      style={[__style.typeButtonView]}>
                      {icons.video(
                        postType == 'video' ? colors.primary : colors.white,
                        17,
                      )}
                    </TouchableOpacity>
                    ...
                    {isPollAllowed && (
                      <TouchableOpacity
                        ...
                        style={__style.typeButtonView}>
                        {icons.poll(
                          postType == 'poll' ? colors.primary : colors.white,
                          17,
                        )}
                      </TouchableOpacity>
                    )}
                    ...
                    {isSurveyAllowed && (
                      <TouchableOpacity
                        ...
                        style={__style.typeButtonView}>
                        {icons.survey(
                          postType == 'survey' ? colors.primary : colors.white,
                          17,
                        )}
                      </TouchableOpacity>
                    )}
                  ...
                  {!isCosmos && showEventOption && (
                    <TouchableOpacity
                      ...
                      style={__style.typeButtonView}>
                      {icons.calendarTick(
                        isEventViewComplete ? colors.primary : colors.white,
                        20,
                      )}
                    </TouchableOpacity>
                  )}
          ...
          <View>
            {!hideLevelView && (
              <Pressable
                ...
                <MyText type={'medium'}>
                  {isCosmos
                    ? feedLevel.split('_').join(' ')
                    : communityLevelWithAllObj[feedLevel]}
                </MyText>
