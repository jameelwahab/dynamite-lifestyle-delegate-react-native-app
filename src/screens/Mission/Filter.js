...
const Filter = ({route, navigation}) => {
  ...
  const [showStart, setShowStart] = useState(
    !!route.params.filters?.from_start_date,
  );
  const [showEnd, setShowEnd] = useState(!!route.params.filters?.to_end_date);
  const [showAttract, setShowAttract] = useState(
    !!route.params.filters?.coins_from,
  );
  const [badges, setBadges] = useState(
    !!route.params.filters?.badges ? route.params.filters?.badges : [],
  );
  const [filter, setFilter] = useState(route.params.filters);
  const [coins, setCoins] = useState({
    from: route.params.filters?.coins_from || '0',
    to: route.params.filters?.coins_to || '0',
  });
  ...
  const handleSelect = item =>
    item.key != 'all'
      ? setFilter({...filter, mission_status: item.key, status: item.title})
      : setFilter({...filter, mission_status: null, status: null});
  ...
  const handleSelect3 = item =>
    setFilter({
      ...filter,
      badge_type: item.key,
      filter_member_title: item.title,
    });
  ...
  const filterList = () => {
    if (badges.length == access.badge_levels.length) {
      return [{title: STRINGS.MISSION_FILTER.noOptions}];
    } else if (badges.length == 0) {
      return [...access?.badge_levels];
    } else {
      return access?.badge_levels.filter(
  ...
  return (
    <RootView title={STRINGS.MISSION_FILTER.title}>
      <MyKeyboardAvoidingView
        style={styles.container}
        showsVerticalScrollIndicator={false}>
        <MyTouchableInput
          label={STRINGS.MISSION_FILTER.status}
          value={filter?.status || STRINGS.MISSION_FILTER.all}
          icon={() => icons.down()}
          onPress={optionStatus}
        />

        <MyTouchableInput
          label={STRINGS.MISSION_FILTER.sortBy}
          value={
            !!filter?.sort_by_coins
              ? filter?.sort_by_coins === 'ascending'
                ? STRINGS.MISSION_FILTER.lowToHigh
                : STRINGS.MISSION_FILTER.highToLow
              : ''
          }
          icon={() => icons.down()}
        ...
        <MyTouchableInput
          label={STRINGS.MISSION_FILTER.filterMemberByBadgeLevel}
        ...
        <MyTouchableInput
          label={
            (filter?.badge_type == 'accept_time' &&
              filter?.filter_member_title) ||
            STRINGS.MISSION_FILTER.currentUserBadgeLevel
          }
        ...
        <MyCheckBox
          title={STRINGS.MISSION_FILTER.searchByStartDate}
          value={showStart}
          onPress={() => {
        ...
        <Collapsible collapsed={!showStart}>
          <MyTouchableInput
            label={STRINGS.MISSION_FILTER.startDateFrom}
            value={filter?.from_start_date || ''}
            icon={() => icons.calendar(colors.primary)}
            onPress={() =>
              ref_calendar?.current?.openModal(
                filter?.from_start_date,
                'from_start_date',
              )
            }
          />
          <MyTouchableInput
            label={STRINGS.MISSION_FILTER.endDateFrom}
            value={filter?.to_start_date || ''}
            icon={() => icons.calendar(colors.primary)}
            onPress={() =>
              ref_calendar?.current?.openModal(
                filter?.from_end_date,
                'to_start_date',
              )
            }
          />
        ...
        {route.params.item.type == 'quest' && (
          <>
            <MyCheckBox
              title={STRINGS.MISSION_FILTER.searchByEndDate}
              onPress={() => {
                setShowEnd(!showEnd);
        ...
        <MyCheckBox
          title={STRINGS.MISSION_FILTER.searchByAttractedCoins}
          value={showAttract}
          onPress={() => {
        ...
        <Collapsible collapsed={!showAttract}>
          <View style={styles.coinInputRow}>
            <View style={styles.flex1}>
              <MyInputs
                label={STRINGS.MISSION_FILTER.coinFrom}
                value={coins.from}
                onChangeText={text => setCoins({...coins, from: text})}
                keyboardType="number-pad"
              />
            </View>
            <View style={styles.coinToWrapper}>
              <MyInputs
                label={STRINGS.MISSION_FILTER.coinTo}
                value={coins.to}
        ...
        <View style={styles.buttonRow}>
          <MyClearButton
            ...
            title={STRINGS.MISSION_FILTER.clearFilter}
            onPress={() => {
              setFilter({});
              setShowStart(false);
              setShowEnd(false);
          ...
          <MyButton
            style={styles.flex1}
            title={STRINGS.MISSION_FILTER.submit}
            onPress={() => {
              if (
                (!!filter?.badge_type && badges.length == 0) ||
                (badges.length != 0 && !!filter?.badge_type == false)
              ) {
                showToast({
                  body: STRINGS.MISSION_FILTER.pleaseSelectFilterMember,
                  title: STRINGS.MISSION_FILTER.filterBadgeLevelNotSelected,
                });
              } else {
                nav.navigate(routes.missionMemberList, {
                  filter: {
        ...
        <CalendarModal
          ...
          onDateSelected={(date, type) => {
            if (type?.includes('date'))
              setFilter({
                ...filter,
                [type]: moment(date).format(dateTimeFormat.date2),
              });
