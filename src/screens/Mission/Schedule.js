  ...
  return (
    <View style={styles.container}>
      <View style={styles.heading_container}>
        <View style={styles.headingRow}>
          ...
          <MyText
            ...
            {res?.mission_schedule?.main_heading ||
              route?.params?.heading ||
              STRINGS.SCHEDULE.defaultHeading}
          </MyText>
        </View>
      ...
      <FlatList
        ...
        ListFooterComponent={<View style={styles.spacer50} />}
        showsVerticalScrollIndicator={false}
        renderItem={({item, index}) =>
          !loading && (
            <>
              <ScheduleView schedule={res?.mission_schedule} index={index} />
            </>
          )
        }
      />
...
export const HeaderView = ({
  ...
  if (type == 'quest' && !!embed_code) {
    return (
      <MyWebview width={utilities.screenWidth() - 20} html={embed_code || ''} />
    );
  } else if (!!video_url) {
    return video_url?.includes('vimeo') ? (
      <VimeoWithPip url={video_url} focused={focuse} id={mission_id} />
    ) : (
      <WebPlayer
    ...
    return <ResponsiveImage2 uri={S3_URL + img_url} />;
  }
};
...
const Overview = ({res}) => {
  return (
    <View>
      ...
      <View style={styles.sched_img_container}>
        <ItemCountView
          text1={`${res?.mission_schedule?.total_number_of_days} ${STRINGS.SCHEDULE.day}`}
          backgroundColor={colors.secondary}
          text2={STRINGS.SCHEDULE.schedule}
          img={require('../../assets/icons/calendar.png')}
        />
        <ItemCountView
          text1={`${res?.mission_schedule?.reward_coins}`}
          backgroundColor={colors.secondary}
          text2={STRINGS.SCHEDULE.coinsRewards}
