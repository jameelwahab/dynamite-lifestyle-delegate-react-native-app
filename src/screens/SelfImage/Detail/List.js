  ...
  const renderList = ({ item, index }) => {
    return (
      <View style={__styles.itemRootView}>
        ...
        <View>
          <MyText color={colors.primary} >{item?.action_by_info}</MyText>
          <View style={{ marginTop: 5 }}>
            <MyText fontSize={12} color={colors.white} >{moment(item?.message_date_time).format(dateTimeFormat.dateTime)}</MyText>
          </View>
        </View>
      </View>
    )
