...
const ChatModal = ({
  ...
  const getOldChatFromServer = async () => {
    let res = await GET_PORTAL_EXISTING_CHAT_BY_VIDEO_ID({
      token,
      navigation,
      videoId,
      page: 0,
    ...
    if (res.code == 200) {
      setList(
        page == 0
          ? res?.dynamite_event_category_video_chat
          : [...list, ...res?.dynamite_event_category_video_chat],
      );
      setPinList(res?.dynamite_event_category_video_featured_chat);
    ...
    for (const item of OptionList) {
      ...
      if (type === 'message') {
        if (isMe) {
          ...
          continue;
        } else if (comment?.action_by == 'consultant') {
          continue;
        }
      }
