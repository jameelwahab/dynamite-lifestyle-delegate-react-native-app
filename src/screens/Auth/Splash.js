...
const Splash = ({navigation}) => {
  ...
  const checkAuth = async () => {
    try {
      ...
      if (token != null) {
        ...
        if (resp?.code == 'error') {
          Alert.alert('Something went wrong', resp?.message, [
  ...
  return (
    <SafeAreaView
      ...
      <Animated.View
        style={[{marginHorizontal: 40, alignItems: 'center'}, textOpacity]}>
        <Image
          source={icons.missionControl}
          style={{width: utilities.screenWidth() - 80}}
          resizeMode="contain"
        />
